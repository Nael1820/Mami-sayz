function onCreate()
	...
	for i = 0, getProperty('unspawnNotes.length')-1 do
		...
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'Holy_Note' then
			setPropertyFromGroup('unspawnNotes', i, 'texture', 'Holy_Note'); --Change texture
...
function goodNoteHit(id, noteData, noteType, isSustainNote)
	if noteType == 'Holy_Note' then
		...
		local animToPlay = '';
		if noteData == 0 then
			animToPlay = 'shootLEFT';
		elseif noteData == 1 then
			animToPlay = 'shootDOWN';
		elseif noteData == 2 then
			animToPlay = 'shootUP';
		elseif noteData == 3 then
			animToPlay = 'shootRIGHT';
		end
		characterPlayAnim('dad', animToPlay, false);
...
function onUpdate(elapsed)
	if healthDrain > 0 then
		...
		setProperty('health', getProperty('health') - 0 * elapsed);
		if healthDrain < 0 then
			healthDrain = 0;
		end
	end
